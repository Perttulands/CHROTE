...
AgentArena runs as containers orchestrated by Docker Compose:

- **agent-arena**: nginx reverse proxy, ttyd web terminal, Node API, SSH server
- **tailscale-arena**: network sidecar providing Tailnet access
...
- A text filter narrows sessions by name.
...
**Native file browser**
- Browse mounted volumes exposed through file API.
- List and grid view.
...
- Add clear health indicators for: API reachable, ttyd reachable.
