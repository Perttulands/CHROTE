...
# startWireDrag / drawWires). Governing principle (D7): connect anything to anything and it just
# works. Connections are STRUCTURE (round-trip files↔CLI↔UI); a hand-routed lane is LAYOUT only.

Feature: Connect nodes — anything to anything, and it just works
  As an agent authoring a graph (or a human tweaking one)
  I need to wire outputs to inputs freely, reconnect, branch, join, and unwire
  ...
  Scenario: A gate routes from distinct pass and fail outputs
    ...
    And I drag from "gate:fail" to "frame:in[0]"
    Then "gate:pass -> ship:in[0]" and "gate:fail -> frame:in[0]" both exist
    And the fail wire pointing back to "frame" is a pushback loop (allowed)
    # The auto-route graph stays acyclic; the cycle exists only via the explicit wire.
  ...
  @ui @file
  Scenario: A node accepts multiple incoming connections (a JOIN)
    When I wire "frame:out[0] -> ship:in[0]"
    And I add a second input port to "ship"
    And I wire "research:out[0] -> ship:in[1]"
    Then "ship" has two incoming connections
  ...
  Scenario: JOIN fan-in is explicit by input port
    ...
    Then the writer rejects the second edge to that formation input port with a conflict
