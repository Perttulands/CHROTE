...
func TestRunCommandRecordCodecRejectsStatePayloadAndHashContradictions(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "applied missing run id", raw: testAuthorityRemoveField(applied, `,"runId":"`+testAuthorityRecordRunID+`"`)},
		{name: "applied has rejection code", raw: testAuthorityInsertBefore(applied, `,"runId":`, `,"rejectionCode":"extra"`)},
