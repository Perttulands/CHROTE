...
func TestS5EscalationSentinelRecordsLedgerAndProjection(t *testing.T) {
	...
	if escalation.NodeID != "fmn_work" {
		t.Fatalf("escalation node = %q, want fmn_work", escalation.NodeID)
	}
	...
	if open[0].RunID != started.RunID || open[0].NodeID != "fmn_work" || open[0].Reason != "found a better direction" || open[0].Severity != "needs-attention" {
