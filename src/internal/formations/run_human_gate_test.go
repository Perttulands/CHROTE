...
// TestS5HumanGateFailPushbackResumeReDispatchesWork locks home-28ww: a human
// gate FAIL wired back to the work formation (gate:fail -> work:in) routes
// correctly, but archon run resume failed loud with resume_no_work because
// resumeSnapshot marked the already-run work node completed and the dispatch
// loop skipped the re-routed node. Resume must re-dispatch the fail-routed
// (pushback) target so the human-in-the-loop revise/retry cycle works, then a
// later approve resumes through the pass wire to success.
func TestS5HumanGateFailPushbackResumeReDispatchesWork(t *testing.T) {
	...
	// Human rejects -> fail wire pushes the work formation back.
	status, err = engine.RecordHumanGateVerdict(status.RunID, HumanGateVerdictRequest{
		GateID:  "gate_review",
		Verdict: "fail",
	...
	// Resume MUST re-dispatch the work formation (home-28ww: previously resume_no_work).
	executor.calls = nil
	...
	for _, ev := range events {
		if ev.Type == RunEventError && ev.Data["code"] == "resume_no_work" {
			t.Fatalf("resume recorded resume_no_work; fail-routed work was not re-queued (home-28ww)")
		}
	}
	if got, want := executor.nodeIDs(), []string{"fmn_work"}; !reflect.DeepEqual(got, want) {
	...
	// Full revise cycle: approve -> resume -> ship -> succeeded.
	status, err = engine.RecordHumanGateVerdict(status.RunID, HumanGateVerdictRequest{
		GateID:  "gate_review",
		Verdict: "pass",
