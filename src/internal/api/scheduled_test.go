...
func TestScheduledTmuxRunnerSendsPromptWithLiteralSendKeysArgv(t *testing.T) {
	tmpDir := t.TempDir()
...
`
	...
	wantFirst := []string{"-S", "/tmp/chrote-fake-tmux.sock", "send-keys", "-t", "ops", "-l", "--", prompt}
	wantSecond := []string{"-S", "/tmp/chrote-fake-tmux.sock", "send-keys", "-t", "ops", "Enter"}
	if len(calls) != 2 || strings.Join(calls[0], "\x00") != strings.Join(wantFirst, "\x00") || strings.Join(calls[1], "\x00") != strings.Join(wantSecond, "\x00") {
		t.Fatalf("tmux calls = %#v, want %#v then %#v", calls, wantFirst, wantSecond)
	}
}
...
func (r *fakeScheduledRunner) SendPrompt(_ context.Context, target scheduled.Target, prompt string) error {
