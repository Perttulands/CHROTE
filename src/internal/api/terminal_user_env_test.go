...
// servers. The server must refuse to start instead of silently picking one.
func TestValidateTerminalUserEnv_RejectsDuplicateUserSocketKey(t *testing.T) {
	t.Setenv("CHROTE_TMUX_SOCKET", "alice=/run/user/2001/chrote-tmux/tmux-1000/default,build=/tmp/tmux-2002/default,alice=/run/user/2001/chrote-alt-tmux/default")
	t.Setenv("CHROTE_TERMINAL_USER_WORKDIRS", "")

	err := ValidateTerminalUserEnv()
	if err == nil {
		t.Fatal("duplicate alice entry in CHROTE_TMUX_SOCKET was accepted; the server would start with listing and attach resolving different sockets")
	}
	message := err.Error()
...
func TestValidateTerminalUserEnv_RejectsDuplicateWorkdirKey(t *testing.T) {
	t.Setenv("CHROTE_TMUX_SOCKET", "alice=/tmp/tmux-2001/default")
...
func TestValidateTerminalUserEnv_RejectsNonAbsoluteSocket(t *testing.T) {
	t.Setenv("CHROTE_TMUX_SOCKET", "alice=relative/default")
	t.Setenv("CHROTE_TERMINAL_USER_WORKDIRS", "")

	err := ValidateTerminalUserEnv()
	if err == nil || !strings.Contains(err.Error(), "absolute") {
		t.Fatalf("relative CHROTE_TMUX_SOCKET error = %v, want an absolute-path rejection", err)
	}
}
