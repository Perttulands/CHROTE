...
func TestTmuxHandler_ListSessionsFiltersReservedProbeSessions(t *testing.T) {
	installScriptedTmux(t, `
case "$*" in
  *list-sessions*) printf '$1\tchrote-probe-123\t1\t0\t/workspaces/alice\tbash\t1\t120\t40\tlatest\t1\t\t1756900000\ton\n$2\twork\t1\t0\t/workspaces/alice\tbash\t1\t120\t40\tlatest\t1\t\t1756900000\ton\n' ;;
esac
`)
	t.Setenv("CHROTE_TMUX_SOCKET", "alice=/tmp/tmux-a")
...
func TestTmuxHandler_ListSessionsReportsLiveActivePaneCWDAndCommand(t *testing.T) {
	installScriptedTmux(t, `
case "$*" in
  *pane_current_path*) printf '$9\twork\t1\t0\t/workspaces/alice/live\tcodex\t1\t120\t40\tlatest\t1\t\t1756900000\ton\n$10\tno-cwd\t1\t0\t\tbash\t1\t120\t40\tlatest\t1\t\t1756900000\ton\n' ;;
esac
`)
	t.Setenv("CHROTE_TMUX_SOCKET", "alice=/tmp/tmux-a")
...
func TestTmuxHandler_ListSessionsAggregatesConfiguredTerminalUsers(t *testing.T) {
	...
	script := `#!/bin/sh
case "$*" in
  *"/tmp/tmux-p"*) printf '$1\talice-shell\t1\t0\t/home/operator\tbash\t1\t120\t40\tlatest\t1\t\t1756900000\ton\n' ;;
  *"/tmp/tmux-t"*) printf '$2\tbuild-shell\t2\t1\t/home/secondary\tbash\t1\t120\t40\tlatest\t1\t\t1756900000\ton\n' ;;
  *) printf '$3\tunexpected\t1\t0\t/tmp\tbash\t1\t120\t40\tlatest\t1\t\t1756900000\ton\n' ;;
esac
`
