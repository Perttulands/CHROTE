...
At the time of this drill, Gas City restart reconciliation looked acceptable
for the then-current CHROTE 3.0 central-workflow pilot if CHROTE treated
`gc session peek` or a CHROTE-side adapter as the transcript recovery path for
tmux-backed sessions.

This recommendation is historical. ADR-0003 rolled back active CHROTE/Gas City
integration; do not treat this drill as a standing implementation plan.
