...
```bash
cd src
GOTOOLCHAIN=go1.26.5 gofmt -w <changed-go-files>
GOTOOLCHAIN=go1.26.5 go vet ./...
GOTOOLCHAIN=go1.26.5 go test ./...
GOTOOLCHAIN=go1.26.5 go test -race ./...
GOTOOLCHAIN=go1.26.5 go test -coverprofile=coverage.out ./...
...
- managed files and the user unit are written under the selected prefix;
...
It validates the generated unit with `systemd-analyze`. It deliberately does not start or replace a real user service.
