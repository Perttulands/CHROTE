...
- Gas City provides documented primitives needed for identity access:
  `gc session new <template> --alias <name> --title <title> --no-attach --json`
  and `gc session attach <identity-or-id>`.
...
- The dashboard session creation UI lets the user choose:
  ...
  - Gas City identity with name/alias, template, and optional title.
...
- Only native `gc session new` counts as identity creation.
