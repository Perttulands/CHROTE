...
- A tile is in exactly one of four states: `Idle` (bound, off screen), `Live` (attached and sizing), `Taken over` (session alive under another client; last frame plus `Reclaim`), `Ended` (session gone; last frame plus `Restart` and `Remove`). A tile that has been shown keeps its connection and its last rendered frame off screen, so do not add a disconnect on hide.
- A tile attaches with `attach-session -d` and is that session's one sizing client. Peek attaches with `-f ignore-size`, owns its own terminal, and never displaces another client.
