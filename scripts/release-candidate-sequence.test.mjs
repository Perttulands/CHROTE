...
test('candidate caller fails closed when canonical worktree inspection fails', async () => {
  const checkout = await mkdtemp(join(tmpdir(), 'chrote-release-candidates-status-failure-checkout-'))
  const bin = await mkdtemp(join(tmpdir(), 'chrote-release-candidates-status-failure-tools-'))
...
`)
  await writeFile(join(bin, 'git'), `#!/usr/bin/env bash
set -euo pipefail
if [ "$1" = '-C' ] && [ "$2" = '${checkout}' ] && [ "$3" = 'status' ]; then
  if [ "$#" -ne 4 ] || [ "$4" != '--porcelain' ]; then
    echo 'unexpected canonical git status invocation' >&2
    exit 74
  fi
  echo 'injected canonical git status failure' >&2
  exit 73
fi
exec /usr/bin/git "$@"
`)
  ...
  await assert.rejects(
    execFile('bash', [join(checkout, 'scripts/build-release-candidates.sh')], {
      cwd: checkout,
      env: {
        ...process.env,
        PATH: `${bin}:${process.env.PATH}`,
    ...
    (error) => {
      ...
      assert.match(error.stderr, /injected canonical git status failure/)
      return true
    },
  )

  await assert.rejects(readFile(join(checkout, 'dist/chrote-server-linux-amd64')), { code: 'ENOENT' })
  await assert.rejects(readFile(join(checkout, 'dist/chrote-server-linux-arm64')), { code: 'ENOENT' })
})
