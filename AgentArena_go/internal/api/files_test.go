...
func TestFilesHandler_NewFilesHandler(t *testing.T) {
	...
	if len(handler.allowedRoots) != 2 {
		t.Errorf("Expected 2 allowed roots, got %d", len(handler.allowedRoots))
	}
}
...
func TestFilesHandler_ListRoot(t *testing.T) {
	...
	if len(response.Items) != 2 {
		t.Errorf("Expected 2 root items, got %d", len(response.Items))
	}

	// Check that items are code and vault
	...
	if !names["code"] || !names["vault"] {
		t.Errorf("Expected code and vault in root, got %v", names)
	}
}
